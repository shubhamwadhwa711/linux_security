    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if response.get('status_code') < 400 or response.get('status_code') in VALID_HTTP_STATUS_CODES:
                    yield {'is_broken': False, 'status_code': response.get('status_code'), 'url': url}
                else:
                    yield {'is_broken': True, 'status_code': response.get('status_code'), 'url': url}
